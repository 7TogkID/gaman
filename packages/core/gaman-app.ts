...
export class GamanApp<A extends AppConfig = any> {
  ...
  registerIntegration(integrationFactory: IntegrationFactory<A>) {
  ...
  constructor(mainBlock: Block<A>) {
     ...
     */
    if (this.#blocks.some((b) => b.path === mainBlock?.path)) {
  ...
  private registerBlock(block: Block<A>) {
