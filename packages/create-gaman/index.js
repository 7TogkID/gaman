...
async function getPackageLatest() {
  return await fetch("https://registry.npmjs.org/gaman/latest", {method: "GET"}).then((r) => r.json());
}
...
`);
  ...
  try {
    ...
    console.log(
      `✅ Template "${subfolder}" copied successfully to ${answers.projectName}`
    );
  } catch (err) {
  ...
  console.log(`✅ Updated package.json with project name "${packageJson.name}" and gaman version ^${latestGaman}.`);
}
