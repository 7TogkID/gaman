 ...
 */
export function parseRateLimit(context, options) {
    let headers = {};
...
function parseHeaders(headers, options) {
    ...
    const used = toInt(getHeader(headers, `${prefix}used`)) ??
...
function parseCombinedHeader(header) {
...
function parseReset(input, type) {
    if (!input)
        return;
