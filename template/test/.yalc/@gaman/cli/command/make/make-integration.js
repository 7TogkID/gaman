...
class MakeIntegration extends Command {
    constructor() {
        super('make:integration', 'Generate a integration template', 'gaman make:integration <name>', ['make:i']);
    }
    async execute(args) {
...
function placeIntegrationToMainFile(pathName) {
    ...
    const bootstrapRegex = /defineBootstrap\([^,]+,\s*async\s*\(\s*app\s*\)\s*=>\s*\{\s*/;
