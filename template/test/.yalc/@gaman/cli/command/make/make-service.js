...
class MakeService extends Command {
    constructor() {
        super('make:service', 'Generate a service template', 'gaman make:service <name>', ['make:s']);
    }
    async execute(args) {
        ...
        filePath = _path.join(process.cwd(), `src/module/${path}`, `${name}.service.ts`);
...
function placeToDataServiceBlock(pathName) {
    ...
    try {
        ...
        if (match) {
            ...
            fs.writeFileSync(`${SRC_DIR}/module/${path}/${name}.block.ts`, blockContent);
        }
        else {
