...
import { formatPath, isHtmlString, removeEndSlash, } from '@gaman/common/utils/utils';
...
export class GamanApp {
    get blocks() {
        return __classPrivateFieldGet(this, _GamanApp_blocks, "f");
    }
    get integrations() {
        return __classPrivateFieldGet(this, _GamanApp_integrations, "f");
    }
    get server() {
        return __classPrivateFieldGet(this, _GamanApp_server, "f");
    }
    get strict() {
        return __classPrivateFieldGet(this, _GamanApp_strict, "f");
    }
    /**
     ...
     */
    setStrict(v = true) {
        __classPrivateFieldSet(this, _GamanApp_strict, v, "f");
    }
    /**
     ...
     */
    registerIntegration(...integrationFactories) {
        for (const factory of integrationFactories) {
            const integration = factory(this);
            __classPrivateFieldGet(this, _GamanApp_integrations, "f").push(integration);
            integration.onLoad?.();
        }
    }
    constructor(mainBlock) {
        ...
        __classPrivateFieldSet(this, _GamanApp_websocket, new GamanWebSocket(this), "f");
    }
    getBlock(blockPath) {
        const path = formatPath(blockPath, __classPrivateFieldGet(this, _GamanApp_strict, "f"));
        const block = __classPrivateFieldGet(this, _GamanApp_blocks, "f").find((b) => formatPath(b.path || '/', __classPrivateFieldGet(this, _GamanApp_strict, "f")) === path);
        return block;
    }
    registerBlocks() {
        /**
         ...
         */
        if (__classPrivateFieldGet(this, _GamanApp_blocks, "f").some((b) => b.path === this.mainBlock?.path || '/')) {
        ...
        function initChilderns(basePath, childrens, app) {
            for (const blockChild of childrens) {
                ...
                if (__classPrivateFieldGet(app, _GamanApp_blocks, "f").some((b) => b.path === childPath)) {
        ...
        // * init childrens
        initChilderns(this.mainBlock.path || '/', this.mainBlock.blocks || [], this);
        const endTime = performance.now();
        Log.info(`${__classPrivateFieldGet(this, _GamanApp_blocks, "f").length} Blocks have been registered §a(${(endTime - startTime).toFixed(1)}ms)§r`);
        // * tambahin "/" di belakang kalau strict
        function register(block, app) {
            block.path = formatPath(block.path || '/', __classPrivateFieldGet(app, _GamanApp_strict, "f"));
            __classPrivateFieldGet(app, _GamanApp_blocks, "f").push(block);
        }
    }
    async requestHandle(req, res) {
        ...
        try {
            const blocksAndIntegrations = sortArrayByPriority([...__classPrivateFieldGet(this, _GamanApp_blocks, "f"), ...__classPrivateFieldGet(this, _GamanApp_integrations, "f")], 'priority', 'asc');
            for await (const blockOrIntegration of blocksAndIntegrations) {
                if ('routes' in blockOrIntegration) {
                    ...
                    try {
                        ...
                        if (block.error) {
                            // ! Block Error handler
                            const result = await block.error(new HttpException(403, error.message, error), ctx, next);
                            if (result) {
                                return await this.handleResponse(result, ctx);
                            }
                        }
    ...
    async handleRoutes(routes, ctx, basePath = '/') {
        for await (const [path, handler] of Object.entries(routes)) {
             ...
             */
            const routeFullPath = formatPath(`${basePath}/${path}`, __classPrivateFieldGet(this, _GamanApp_strict, "f"));
             ...
             */
            const requestPath = __classPrivateFieldGet(this, _GamanApp_strict, "f")
    ...
    async handleResponse(result, ctx) {
        //@ts-ignore
        const res = ctx[HTTP_RESPONSE_SYMBOL];
         ...
         */
        let response = new Response(undefined, { status: 404 });
         ...
         */
        if (__classPrivateFieldGet(this, _GamanApp_integrations, "f")) {
            const integrations = sortArrayByPriority(__classPrivateFieldGet(this, _GamanApp_integrations, "f"), 'priority', 'asc');
    ...
    listen(port = 3431, host = 'localhost', cb) {
        ...
        __classPrivateFieldSet(this, _GamanApp_server, http.createServer(this.requestHandle.bind(this)), "f");
        __classPrivateFieldGet(this, _GamanApp_server, "f").on('upgrade', (request, socket, head) => {
            const urlString = request.url || '/';
            const { pathname } = new URL(urlString, `http://${request.headers.host}`);
            const wss = __classPrivateFieldGet(this, _GamanApp_websocket, "f").getWebSocketServer(pathname);
            if (wss) {
        ...
        __classPrivateFieldGet(this, _GamanApp_server, "f").listen(port, host, cb);
    }
    close() {
        return new Promise((resolve, reject) => {
            if (__classPrivateFieldGet(this, _GamanApp_server, "f")) {
                __classPrivateFieldGet(this, _GamanApp_server, "f").close((err) => {
                    if (err)
                        reject(err);
                    else
                        resolve();
