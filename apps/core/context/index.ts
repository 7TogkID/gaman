...
import { HTTP_REQUEST_SYMBOL, HTTP_RESPONSE_SYMBOL } from '@gaman/common/contants';
...
export async function createContext(
	...
	const ctx: Context = {
		...
		/**
		 * ? set, has, dan get ini akan di set di masing masing route
		 * ? ini fungsi untuk pipe / interceptor
		 * ! kalau mau pakai global kek dari middleware trus di pakai di pipe atau handler pakailah `ctx.locals`
		 * @example
		 * ```ts
		 * route("/")
		 * 	.pipe(AuthGuard)
		 *  .pipe((ctx, error) => {
		 * 		ctx.set("validate-data", {
		 * 			userName: "Angga7Togk"
		 * 		})
		 * 	})
		 * 	.handler((ctx) => {
		 * 		return ctx.get("validate-data")
		 * 	})
		 * ```
		 */
		set: null as any,
